...
/* BEGIN AK CHANGE */
@property (nonatomic, unsafe_unretained) BOOL useCurrentOrientation;
@property (nonatomic, unsafe_unretained) UIInterfaceOrientation nextOrientation;
@property (nonatomic, unsafe_unretained) UIInterfaceOrientation currentOrientation;
//@property (nonatomic, unsafe_unretained) BOOL wasDisplayedAlready;
//@property (nonatomic, unsafe_unretained) BOOL didChangeOrientationSinceDisplayed;
/* END AK CHANGE */
...
{
    self.nextOrientation = toInterfaceOrientation;
    ...
    /* BEGIN AK CHANGE */
    //    if (self.wasDisplayedAlready) {
    //        self.didChangeOrientationSinceDisplayed = YES;
    //    }
    /* END AK CHANGE */
...
{
    self.currentOrientation = self.nextOrientation;
...
- (void)updateTopViewHorizontalCenterWithRecognizer:(UIPanGestureRecognizer *)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan) {
        ...
        if (fabs(translation.x) > fabs(translation.y)) {
            ...
            if ((newCenterPosition < self.resettedCenter && self.anchorLeftTopViewCenter == NSNotFound) ||
                (newCenterPosition > self.resettedCenter && self.anchorRightTopViewCenter == NSNotFound)) {
                
                newCenterPosition = self.resettedCenter;
            }
            
            // BEGIN: THIS IS AN AK EXTENSION
            newCenterPosition = [self correctOverPanningOfCenterPosition:newCenterPosition];
            // END: THIS IS AN AK EXTENSION
 ...
 * BEGIN: THIS IS AN AK EXTENSION
 */
- (CGFloat)correctOverPanningOfCenterPosition:(CGFloat)newCenterPosition {
    if (self.disableOverPanning) {
        CGFloat panAmountFromInitial = self.resettedCenter - newCenterPosition;
        
        if (self.underLeftWidthLayout == ECFixedRevealWidth &&
            panAmountFromInitial < -self.anchorRightRevealAmount) {
            
            newCenterPosition = self.resettedCenter+self.anchorRightRevealAmount;
            
        } else if (self.underRightWidthLayout == ECFixedRevealWidth &&
                   panAmountFromInitial > self.anchorLeftRevealAmount) {
            
            newCenterPosition = self.resettedCenter-self.anchorLeftRevealAmount;
            
        } else if (self.underLeftWidthLayout == ECVariableRevealWidth &&
                   panAmountFromInitial < -(self.screenWidth-self.anchorRightPeekAmount)) {
            
            newCenterPosition = self.resettedCenter+(self.screenWidth-self.anchorRightPeekAmount);
            
        } else if (self.underRightWidthLayout == ECVariableRevealWidth &&
                   panAmountFromInitial > (self.screenWidth-self.anchorLeftPeekAmount)) {
            
            newCenterPosition = self.resettedCenter-(self.screenWidth-self.anchorLeftPeekAmount);
...
{
    // BEGIN: THIS IS AN AK CHANGE
    [self anchorTopViewTo:side withDuration:0.25f animations:nil onComplete:nil];
    // END: THIS IS AN AK CHANGE
}

// BEGIN: THIS IS AN AK CHANGE
- (void)anchorTopViewTo:(ECSide)side withDuration:(NSTimeInterval)duration animations:(void (^)())animations onComplete:(void (^)())complete
// END: THIS IS AN AK CHANGE
{
...
- (void)resetTopView
{
    ...
    [self resetTopViewWithDuration:0.25f animations:nil onComplete:NULL];
    // END: THIS IS AN AK CHANGE
}

// BEGIN: THIS IS AN AK CHANGE
- (void)resetTopViewWithDuration:(NSTimeInterval)duration animations:(void(^)())animations onComplete:(void(^)())complete
// END: THIS IS AN AK CHANGE
{
    [[NSNotificationCenter defaultCenter] postNotificationName:ECSlidingViewTopWillReset object:self userInfo:nil];
    
    // BEGIN: THIS IS AN AK CHANGE
    self.useCurrentOrientation = YES;
    // END: THIS IS AN AK CHANGE
...
{
    return [self screenWidthForOrientation:self.currentOrientation];
}
/* END ADDED FOR AK */
...
{
    /* BEGIN ADDED FOR AK */
    if (self.useCurrentOrientation) {
        return [self screenWidthForCurrentOrientation];
    }
    /* END ADDED FOR AK */
...
- (void)underLeftWillAppear
{
    ...
    [self.underLeftViewController viewDidAppear:NO];
}
...
- (void)underRightWillAppear
{
    ...
    [self.underRightViewController viewDidAppear:NO];
}
...
- (BOOL)topViewHasFocus
{
    return !_underLeftShowing && !_underRightShowing && !self.topViewIsOffScreen;
}
...
- (void)updateUnderLeftLayout
{
    ...
    if (self.underLeftWidthLayout == ECFullWidth) {
        ...
        [self.underLeftView setFrame:bounds];
